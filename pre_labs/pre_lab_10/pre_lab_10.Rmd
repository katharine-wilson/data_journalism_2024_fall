...
I see information about what precincts and voting locations are in PG county and information about the state and projection for map making purposes. Columns: precinct ID, precinct name, congress, legislature, council, school, polling ID, shape length, shape area and shape.  
```{r}
...
This contains standard information from data sets we've seen before and geographic data (shapes).
```{r}
View(pg_precincts)
```
...
A basic precinct map of Prince George's County. There are a few empty precincts (ghost precincts) without people living in them. 
```{r}
pg_precincts |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The name of every county, some other information about the county and the shape of that county. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
It filters to only include Maryland counties. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
A county map of the state of Maryland (which does not have a gap for the bay)
```{r}
md_counties |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
We used the county FP code to filter just for PG County. This shows just a map for the county. 
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This shows the turnout rates by voting type in each precinct.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This joined the turnout rate in each precinct to the precinct data with the geographic information. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Thus shows the total number of people eligible to vote in each precinct in a map of PG County.  
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This shows the number of voters registered for each party in every Maryland County. This shows that Montgomery County has the highest number of registered voters. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This shows the population of each county in Maryland. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This shows the population of each county with geographic shape information. 
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              state = "MD",
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This shows the name of the county in title case
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Thus changes the title of the countiies to title case and removed the word "county" 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This joins the population data with the party registration data and shape data. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This fixes the name of St. Mary's County and Baltimore County so both data sets use the same names. 
```{r}
md_county_population <- md_county_population |>
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This adds a column that shows how many voters there are of  10k people in each county 
```{r}
md_voters_per_10k <- md_county_population |>
  left_join(voters_by_county) |>
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This turned the number of vters in each county per 10k people there are into a map. 
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This made the map a reddish color 
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
This makes the county seperations more easy to see. 
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This adds a column with maryland voters per 10k people. 
```{r}
...
I noticed that there are more unafilliated voters in Frederick and Baltimore Counties, where Maryland elections are usually decided. 
```{r}
ggplot() +

...
There are many different ways the restaurant names are written in this dataset(such as W. MILLAR & CO and W. MILLAR & CO.).These small spelling, punctuation and capitalization differences prevent proper grouping. 
```{r}
...
The refinr package changed the capitalization of the Scarlet Oak to sentence case. I didn't notice any other differences in the first 20 rows, but on the 21st row THE SALT LINE was also changed to sentence case. 
```{r}
...
The refinr function changed all the resturant names to all caps. The function also changed W. Millar & Company Catering to W. MILLAR & CO. CATERING. It also changed the Hilton Washington to Washington Hilton.  
```{r}
cleaned_dc_food |>
...
cleaned_dc_food |>
  ...
  filter(str_detect(city_clean, "^w|^W")) |>
  arrange(city_clean)
```
...
It does not vary substantially from the previous use of key_collision merge. (I also assumed that this was supposed to be cities that start with W, because there were no cities that started with A)
```{r}
cleaned_dc_food <- dc_food |>
...
cleaned_dc_food <- dc_food |>
  ...
  select(city, city_clean, everything())

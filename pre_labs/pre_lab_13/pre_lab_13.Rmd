...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**Answer**
The first few names are cute pet names, although they are not very related to lemurs. The last 10 names seem to be veering even farther off from the request to names like "Zuzuette". 
```{r}
...
**Answer**
Llama did get the elements right. This code would allow the New York Times to identify data off of thousands of other wedding announcements. 
```{r}
...
**Answer**
Llama did get it right. I would add "key duties" as a column. 
```{r}
...
**Answer**
This response mostly makes sense but it does get a little too technical. 
```{r}
...
**Answer**
The output does make sense and would help me understand the data by giving me the different options of how I can look into the data. 
```{r}
...
**Answer**
I think groq did a good job identifying that there was a military parachustist in mid-air on a blue sky day. The AI did mess up and say that the parachute (with an American flag printed on it) was a real flag that was neatly folded. 

### Task 8: Run a model on your laptop!

Go to <https://ollama.com/> and download and install the Ollama software. Then, in the Terminal (Mac) or PowerShell (Windows), type the following: llama run llama3.2o
...
**Answer** 
This code did not work, I think I might be having a computer problem.
```{r}
emails |>

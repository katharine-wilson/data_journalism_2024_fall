...
**A2**
Talbot County and Worcester County are the two counties with a median age of at least 50. 
```{r}
md_over_50 <-md |>
  filter(estimate > 50)
```
...
```{r}
frederick_together <- frederick_cvr|>
left_join(frederick_precincts, join_by(precinct == precinct))
```
...
**A3**
11,871 voted for different parties in the governr and comptroller race. 95,221 voters voted for the same party in both races. More than 11% of voters split their ticket in these two races. 89,916 people voted for the same party in all five contests. This represented 83.79% of all voters. 
```{r}
split_tickets <- frederick_together |>
  filter(governor_lt_governor!= comptroller)
```

```{r}
solid_tickets <- frederick_together |>
  filter(governor_lt_governor== comptroller)
```
...
```{r}
perfect_solid_tickets <- frederick_together |>
...
```{r}
no_cox_republicans <- frederick_together |>
  filter(governor_lt_governor == "DEM", attorney_general == "REP", u_s_senator == "REP", representative_in_congress_6 == "REP", comptroller == "REP")
```

```{r}
zip_no_cox_republicans <- no_cox_republicans |>
  group_by(zipcode)|>
  summarize(
    count=n()
  )|>
...
**A5**
The top four zip codes with non-Cox Republicans are all in the competitive area of Frederick County. The three zip codes with the fewest non-Cox Republicans are also in Frederick County. The medium ages of the non-cox Republican zip codes are older than the ones with fewer non-Cox Republicans. The areas with more non-Cox Republicans are also near or inside the city of Frederick, whereas fewer non-Cox Republicans are in the more rural areas in the county. 

```{r}
mdzip <- get_acs(geography = "zcta",
              variables = c(medincome = "B01002_001"),
              state = "MD",
              year = 2019)
```
...
```{r}
zip_no_cox_republicans <- zip_no_cox_republicans|>
  mutate(zipcode = as.character(zipcode))
```


```{r}
zip_info_no_cox_republicans <- zip_no_cox_republicans|>
  left_join(mdzip, join_by(zipcode == GEOID))|>
  arrange(desc(count))
```

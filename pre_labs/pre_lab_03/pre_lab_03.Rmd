...
I think R is describing a problem with the data type of the 12 and 13 columns. R was expecting true/false responses but the columns contain numbers. 
```{r}
...
It does not show any parsing errors. 
```{r}
...
The :district" and "mail" columns are number columns, with a data type of "dbl". The "precint" feild has a character data type. 
```{r}
...
There are 402,345 results where the number of mail-in votes are listed as N/A. This implies that there are that many precincts where mail-in vote counts are not reported. 
```{r}
...
There are 12 registrations in January 1900. I think it suggests that there are some data points that were entered incorrectly or voters that should be taken off of the voter roll. 
...
A lot of these voters are unable to vote in the upcoming election such as felons, deceased people and people who have moved. We should probably remove these people from the voter file for analysis of voters. 
...
Character, but it should be number. 
```{r}
glimpse(conowingo)
```
...
**Answer**
Some zip codes listed are ZIP and some are ZIP+4. There are some misspelled names and incorrect capitalization in the city column. The addresses are also not formatted the same 
```{r}
conowingo
```
...
This cleans up the column names to all being in lowercase,with underscores instead of spaces and without a number starting a column name. 
```{r}
# cleaning function
...
The error says "error in `sum()`: ! invalid 'type' (character) of argument". I think this error means that we cannot sum a character column.
```{r}
# cleaning function
...
2 duplicate rows for donor Derrick Hamilton. 
```{r}
cleaned_conowingo |> 
  get_dupes()
```
...
The zip codes were changed to include only the first five digits.
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
...
At least two mispellings remain. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 

...
I noticed that the columns are all character values and that the names of the columns are very long. 
...
I think 2 through 6 represent the column number (positions) of each data set so that way when we rename the columns we don't have to type out the full column names. 
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <li> tags there are in the HTML source? **Answer**
55
```{r}
...
The committee names, section titles, other words, spaces etc. 
```{r}
...
This makes a new dataframe with just the committee url and committee name without any excess white space. It still includes outputs that aren't the committee names. 
```{r}

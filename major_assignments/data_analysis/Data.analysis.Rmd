...
2. DONE Which precincts have the highest rates of early voting of people who voted? 
3. DONE How does early voting participation in Maryland for Governor compare to presidential elections? 
4. DONE For the precincts that swayed the most towards Trump from 2020 to 2024, what is their average income? 
...
First, we need to find the precincts with the highest rates of election day voting and mail in voting in 2024. 
...
```{r}
voting_counts <- read_csv("PG24_AllPrecincts.csv")
```
...
voting_count_sum <- voting_counts |>
  group_by(`County Name`, `Election District - Precinct`)|>
  summarise(sum(Mail_In), sum(election_day_votes))
```

This shows the top precincts for people who voted by mail-in
```{r}
top_mail24 <- voting_count_sum |>
  arrange(desc(`sum(Mail_In)`))
```

This shows the top precincts for people who voted on election day
```{r}
top_day <- voting_count_sum |>
  arrange(desc(`sum(election_day_votes)`))
```

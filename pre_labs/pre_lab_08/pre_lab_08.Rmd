...
**Answer**
The top part of the code adds a column that shows how many more unaffiliated voters there are in Maryland in 2024 than in 2022. The second part arranges this number in decending order and then includes counties in the dataframe. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
I see a bar chart with counties in alphabetical order on the x axis and the count on the y axis. 
```{r}
maryland_top_counties |>
  ggplot() +
...
This ordered the counties by the difference in unaffiliated voters. 
```{r}
maryland_top_counties |>
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
This flipped the x and y axis, making it more easy to understand. 
```{r}
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=reorder(County,UNAF_DIFF), weight=UNAF_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
The background color changed along with small formatting improvements. 
```{r}
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=reorder(County,UNAF_DIFF), weight=UNAF_DIFF)) +
...
The background color of the chart changed to blue. 
```{r}
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=reorder(County,UNAF_DIFF), weight=UNAF_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
There is a title now, a data source lsited, and the x and y axis titles are improved. 
```{r}
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=reorder(County,UNAF_DIFF), weight=UNAF_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
I see a list of total contributions per date and the date they were donated. There are too many rows to really see the newsworthy trends that appear in the data.  
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
I see a line graph that shows the number of contributions per date, showing clearly when the number of donations per day peaked. There was a signifigant jump at the end of June. 
```{r}
md_winred_by_date |>
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
This added date breaks to the graphic. This allows people who look at the graphic to get more information out of the graphic and identify when different contribution trends occured during that month. 
```{r}
md_winred_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
This tilted the date lables to make the x axis less confusing. 
```{r}
md_winred_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 

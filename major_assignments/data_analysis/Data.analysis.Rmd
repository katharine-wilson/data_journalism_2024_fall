...
These cities are Detroit, Michigan, Milwaukee and Shelby County (where Memphis is). 
...
Milwakee data calculation data from: https://county.milwaukee.gov/EN/County-Clerk/Election-Commission/Election-Results
...
////////IGNORE FOR NOW///////////
Prince George's County 2020 Total Votes Per Precinct 
```{r}
Q5_PGTotal20 <-Total_2020 |>
  filter(`Office Name` == "President - Vice Pres")|>
  mutate(across(starts_with("Total"), ~ replace_na(. , 0)))|>
  filter(County == 17)|>
  group_by(`Election Precinct`,`Election District`)|>
  summarise(sum(TotalVotes))|>
  rename(TotalVotes = `sum(TotalVotes)`)
```

Prince George's County 2020 Trump Votes Per Precinct 
```{r}
Q5_PGTrump20 <-Total_2020 |>
  filter(`Office Name` == "President - Vice Pres")|>
  filter(`Candidate Name` == "Donald J. Trump")|>
  mutate(across(starts_with("Total"), ~ replace_na(. , 0)))|>
  filter(County == 17)|>
  group_by(`Election Precinct`,`Election District`)|>
  summarise(sum(TotalVotes))|>
  rename(TrumpVotes = `sum(TotalVotes)`)
```

Baltimore County 2020 Total Votes Per Precinct 
```{r}
Q5_BaltTotal20 <-Total_2020 |>
  filter(`Office Name` == "President - Vice Pres")|>
  mutate(across(starts_with("Total"), ~ replace_na(. , 0)))|>
  filter(County == 03)|>
  group_by(`Election Precinct`,`Election District`)|>
...
Baltimore County 2020 Trump Votes Per Precinct 
```{r}
Q5_BaltTrump20 <-Total_2020 |>
  filter(`Office Name` == "President - Vice Pres")|>
  filter(`Candidate Name` == "Donald J. Trump")|>
  mutate(across(starts_with("Total"), ~ replace_na(. , 0)))|>
  filter(County == 03)|>
...
Prince George's County 2024 Total Votes Per Precinct 
```{r}
Q5_PGTotal24 <- total_election_2024|>
  filter(`Office Name` == "President - Vice Pres")|>
  mutate(across(starts_with("Total"), ~ replace_na(. , 0)))|>
  filter(County == 17)|>
  group_by(`Election District - Precinct`)|>
  summarise(sum(Total2024))|>
  rename(TrumpVotes24 = `sum(Total2024)`)
```

Prince George's County 2024 Trump votes Per Precinct 
```{r}
Q5_PGTrump24 <- total_election_2024|>
  filter(`Office Name` == "President - Vice Pres")|>
  filter(`Candidate Name` == "Donald J. Trump and JD Vance")|>
  mutate(across(starts_with("Total"), ~ replace_na(. , 0)))|>
  filter(County == 17)|>

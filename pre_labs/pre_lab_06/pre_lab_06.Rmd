...
Not all of the data is in number form. There are also a lot of N/A results because of headers in the middle of the data. We also don't have any column headers. 
```{r}
...
X29 still has NA values. There are still no easily understood headers.
```{r}
...
The header names are fixed for the first six columns and there is a new column for what office each candidate ran for. There are still problems in X29 with NAs and with all of the rest of the column headers. 
```{r}
benzie_2024 <- benzie_2024 |>

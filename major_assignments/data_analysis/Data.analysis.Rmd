...
md_earlyvote2024 <- early_voting |>
  ...
  mutate(Total2024 = Day1 + Day2 + Day3 + Day4 + Day5 +Day6 + Day7 + Day8)
```

```{r}
md_earlyvote2022 <- early_voting_2022 |>
  mutate(across(starts_with("Day"), ~ replace_na(. , 0)))|>
  mutate(Total2022 = Day1 + Day2 + Day3 + Day4 + Day5 +Day6 + Day7 + Day8)
```
...
```{r}
md_earlyvote_22_24 <- md_earlyvote2022_squeezed |> left_join(md_earlyvote2024_squeezed, join_by(COUNTY_NAME == COUNTY_NAME))
```

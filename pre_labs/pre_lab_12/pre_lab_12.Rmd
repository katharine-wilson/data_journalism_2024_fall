...
This is a dataframe with the url, date, title and text of every press release from Sen. Ben Cardin. 
```{r}
...
This makes each unique word only appear once. 
```{r}
...
This makes every word lowercased and removes punctuation as a word. 
```{r}
...
The graphic shows that Cardin's most used words are the, and, and to. This graphic could be interesting if we eliminated not useful common words. 
...
The top words are more substantive than the origional list of top words. The top words are health, support and act. This is more interesting than before because these words actually give us insight into what Cardin's priorities are. 
```{r}
unique_words |>
...
In 2024, Cardin's top words were Baltimore, support and funding while in 2022 his top words were health, support and community. The 2024 top words likely reflect the collapse of the Baltimore bridge while the 2022 top words are likely due to COVID-19 related bills. 
```{r}
unique_words_2022 |>
  anti_join(stop_words) |>
  group_by(word) |>
...
**Task** Run the following code and describe the results. **Answer**
Running this code allows us to see common topics Cardin talks about in his press releases like COVID 19, health care and the Chesapeake Bay. 
```{r}
releases |>
  filter(year(date) == 2022) |>
...
**Task** Run the following code and describe the results. **Answer**
Heath care is still a top phrase, but it is joined by other phrases important this year like Chesapeake Bay (news-worthy because of serveral bay-related bills Cardin has pushed this year) and Baltimore City (news-worthy because of the bridge collapse). 
```{r}
releases |>
  filter(year(date) == 2024) |>
  unnest_tokens(bigram, text, token = "ngrams", n = 2) |>
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer**
The stem words function turns the list of words into the basic forms of those words. For example, it turns driving into drive. Lemmatize words turns the list of words into the basic forms of words but keeps track of the type of word it is (like nouns stay nouns and verbs stay verbs)
```{r}
...
The lematized words are more useful because they group together words that mean the same thing so we have more useful words appear in the top 10 words no matter the tense. 
```{r}
...
The results show the top words and their sentiments for 2022 and 2024. I think most of the sentiments seem correct, but they could easily be inncorrect depending on the press release. For example, the word "cancer" appears in a lot of press releases and is listed as a negative. But, if there is funding being approved for cancer research or an announcement that someone is cancer-free, that would be a more positive press release. 
```{r}
